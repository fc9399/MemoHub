  ...
  useEffect(() => {
    const handleCallback = () => {
      try {
        ...
        // Send code and state to parent window for connection handling
        // This allows using the parent window's authentication token
        setStatus('success')
        setMessage('Authorization successful, connecting...')

        // Immediately notify parent window to handle connection
        if (window.opener) {
          window.opener.postMessage({
            type: 'oauth-success',
            code,
          ...
          setStatus('error')
          setMessage('Cannot connect to parent window, please open through application page')
        }
      } catch (err) {
