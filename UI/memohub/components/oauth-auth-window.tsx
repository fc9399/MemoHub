...
export function OAuthAuthWindow({ isOpen, onClose, provider, providerName, onSuccess }: OAuthAuthWindowProps) {
  ...
  useEffect(() => {
    ...
    const fetchAuthUrl = async () => {
      try {
        ...
        // Extract redirect_uri from auth URL for subsequent token exchange
        try {
  ...
  const handleAuthorize = () => {
    ...
    // Monitor authorization window close
    const checkClosed = setInterval(() => {
      if (popup?.closed) {
        clearInterval(checkClosed)
        // After window closes, check if authorization was successful
        // Should actually check via callback URL or polling API
        checkAuthStatus()
      }
    }, 500)

    // Listen for messages from auth window (callback page will send postMessage)
  ...
  const checkAuthStatus = async () => {
    try {
      ...
      if (authWindow && !authWindow.closed) {
        try {
          // Try to read popup URL (may fail due to same-origin policy)
          ...
          // Use fallback method: listen for URL changes or prompt user for manual input
          console.log('Cannot access popup URL (cross-origin). Waiting for manual callback...')
          // In actual application, should set up a callback page to handle this
        }
      }
  ...
  const handleAuthSuccess = async (code: string, state?: string, callbackRedirectUri?: string) => {
    try {
      ...
      // Show success state, then let onSuccess callback handle actual connection
